...
function is_valid_toto_set(nums, n, min, max) {
    ...
    function helper(xs) {
        ...
        const val = head(xs);
        highest = bigger(highest, val);
        lowest = smaller(lowest, val);
        counter = counter + 1;
        return helper(tail(xs));
    }
...
function num_of_matches(numsA, numsB) {
    ...
    map(x => remember(x), numsA);
    
    let matches = 0;
    function dont_remember (xs) {
        if (is_null(xs)) {
            return true;
        }
        if (!is_undefined(mem[head(xs)])) {
...
function check_winning_group(bet_nums, draw_nums, extra_num) {
    ...
    // WRITE HERE.
    // group 1 : n matches
    // group 2 : n - 1 matches + extra
    // group 3 : n -1 matches
    // group 4 : n - 2 matches + extra
    // group 5 : n - 2 matches
    const bigger = (x, y) => x > y ? x : y;
    
    const n = length(draw_nums);
    const matches = num_of_matches(bet_nums, draw_nums);
    const extra = is_null(member(extra_num, bet_nums)) ? 0 : -1;
    const group = 2*(n - matches) + 1;
    
    return group >= 6 ? 0 : group + extra;
