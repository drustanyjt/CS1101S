...
function combine(xss) {
    ...
    return accumulate((u, v) => append(u, v), null, xss);
...
function find_gene_end(xs) {
    ...
    const front = pair(() => NaN, xs);
    
    function check(arr_str) {
        function res(ys) {
            if (length(ys) < 3){
                return false;
            }
            return (
                arr_str[0] === head(ys) &&
                arr_str[1] === head(tail(ys)) &&
                arr_str[2] === head(tail(tail(ys))));
        }
        return res;
    }
    
    let checks = list(["T","A","G"], ["T","G","A"], ["T","A","A"]);
    checks = map(check, checks);
    
    function helper(xs) {
        // check if ys has end codon, if it does then set tail to null and end
        const slow = xs;
        const fast = tail(xs);
        let ended = false;
        let i = 0;
    ...
    return helper(front) ? list(tail(front)) : null;
}
